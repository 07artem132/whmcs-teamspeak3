...
class VirtualServerGroupListApi extends ApiValidatorAbstract implements ApiInterface
{
    use ResponseTraits;
    ...
    function run(): void
    {
        ...
        $serverGroupList = $server->serverGroupList(['type' => 1, 'savedb' => 1]);
        ...
        foreach ($serverGroupList as &$group) {
            $group = [
                ...
                'n_modifyp' => $group["n_modifyp"],
                'n_member_addp' => $group["n_member_addp"],
                'n_member_removep' => $group["n_member_removep"],
            ];
        }

        $this->responseData('success', $serverGroupList);
    }
