...
use WHMCS\Module\Addon\TeamSpeak3\Exceptions\FtpException;
use WHMCS\Module\Addon\TeamSpeak3\Exceptions\FtpIsNotDirException;
 ...
 */
class FtpClientController implements Countable
{
    /**
    ...
    public function __construct($connection = null)
    {
        ...
        $this->setWrapper(new FtpWrapperController($this->conn));
    }
     ...
     */
    public function connect($host = null, $ssl = false, $port = null, $timeout = 90)
    {
        if (empty($host)) {
            $host = ModuleConfig::getBackupFtpIp();
        }

        if (empty($port)) {
            $port = ModuleConfig::getBackupFtpPort();
        }
    ...
    {
        if (empty($username)) {
            $username = ModuleConfig::getBackupFtpLogin();
        }

        if (empty($password)) {
            $password = ModuleConfig::getBackupFtpPassword();
        }
     ...
     * @throws FtpIsNotDirException Данной директории не существует
     */
    public function nlist($directory = '.', $recursive = false, $filter = 'sort')
    {
        if (!$this->isDir($directory)) {
            throw new FtpIsNotDirException('"' . $directory . '" is not a directory');
        }
